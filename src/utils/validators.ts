 ...
 * Format: AAAAA9999A
 * 4th char: Entity type (P for Individual, C for Company, etc.)
 */
export const validatePAN = (pan: string, allowedEntities: string[] = ['P']) => {
  const regex = /^[A-Z]{5}[0-9]{4}[A-Z]{1}$/;
  if (!regex.test(pan)) return false;
  const entityType = pan.charAt(3);
  return allowedEntities.includes(entityType);
};
